...
async def get_orcid_person_info(orcid: str, session) -> Dict:
    ...
    Возвращает простой список ссылок с категориями
    """
    if not orcid:
        return {}
...
async def collect_scholar_data(orcid: str) -> Tuple[ScholarProfileAnalyzer, Dict, List[Dict]]:
    ...
    async with aiohttp.ClientSession() as session:
        ...
        print("🔍 Получение DOI из ORCID и OpenAlex...")
        ...
        print("📊 Первичный анализ публикаций для сбора соавторов...")
        ...
        if unique_coauthor_orcids:
            ...
            for orcid_id, person_info in results:
                if person_info and person_info.get('links'):
                    coauthor_profiles[orcid_id] = person_info
                    if SHOW_DEBUG_LOGS:
                        print(f"    ✅ Получен профиль для {orcid_id}: {len(person_info.get('links', []))} ссылок")
        ...
        print("📊 Повторный анализ с учетом профилей соавторов...")
        analyzer.analyze_publications()
    ...
    <head>
        ...
        <style>
            * {{ margin: 0; padding: 0; box-sizing: border-box; }}
            ...
            .coauthor-links-table td {{
                padding: 6px 10px;
                border-bottom: 1px solid #e0e0e0;
                vertical-align: middle;
            }}
            
            .coauthor-links-table tr:hover {{
                background-color: #f5f5f5;
            }}
...
def generate_coauthor_links_html(links: List[Dict], lang: str = 'en') -> str:
    """Генерирует HTML таблицу для ссылок соавтора"""
    if not links:
    ...
    html = """
    <table class="coauthor-links-table">
        <thead>
            <tr>
                <th>Type</th>
                <th>Name / ID</th>
    ...
    for link in links:
        ...
        badge_class = "link-type-other-ids"
        if "Websites" in link_type or "Social" in link_type:
            badge_class = "link-type-websites-social-links"
        
        html += f"""
        <tr>
            <td><span class="link-type-badge {badge_class}">{html.escape(link_type)}</span></td>
            <td>{html.escape(link_name)}</td>
            <td>{link_html}</td>
        </tr>
        """
...
def generate_html_report_with_multiple_authors(all_authors: List[Dict], show_all: bool, journal_logo_base64: Optional[str] = None, app_logo_base64: Optional[str] = None, theme_colors: Optional[Dict] = None, lang: str = 'en') -> str:
    ...
    if show_all:
        for i, author_data in enumerate(authors_to_show):
            ...
            # ====== Генерация карточек соавторов для множественного отчета ======
            coauthors_html = ""
            if top_coauthors_with_orcids:
                for name, data in list(top_coauthors_with_orcids.items())[:15]:
                    count = data.get('count', 0)
